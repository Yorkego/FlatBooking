class FlatsQuery
  ...
  def filter(params_filter)
    ...
    @relation = @relation.where('flats.name ILIKE ?', "%#{params_filter[:name]}%") if params_filter[:name].present?
    ...
    order_by(params_filter[:category], params_filter[:direction])
    # byebug
    @relation
  end
  ...
  def order_by(category, direction)
    ...
    when 'created_at'
      ...
      @relation = @relation.order("flats.created_at #{direction}")
    when 'price'
      # @relation.order("flats.price #{direction}")
      @relation = @relation.order("flats.price #{direction}")
    when 'capacity'
      # @relation.order("flats.capacity #{direction}")
      @relation = @relation.order("flats.capacity #{direction}")
    when 'popularity'
      # @relation.left_outer_joins(:bookings).order("bookings.count #{direction} NULLS LAST").group('flats.id')
      @relation = @relation.left_outer_joins(:bookings).order("bookings.count #{direction} NULLS LAST").group('flats.id')
    end
  end
